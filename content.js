...
(function () {
  ...
  const GW_UNREAD_COUNTS_KEY = 'gwUnreadCounts';
  const GW_MAILBOX_CACHE_KEY = 'gwMailboxCache';
  ...
  let gwUnreadSnapshot = {};
  let gwMailboxCacheSnapshot = [];
  ...
  function getUnreadCountForEntry(entry) {
    ...
    if (entry.emailSource === 'gw') {
      // gw 是 ULID 字符串 id + address 双标识：快填历史里只有地址，
      // 先靠 gwMailboxCacheSnapshot 反查 id，再按 String(id) 索引 gwUnreadSnapshot。
      const cached = (gwMailboxCacheSnapshot || []).find((item) => item?.address === email);
      const count = cached?.id ? gwUnreadSnapshot?.[String(cached.id)] : 0;
      return Number.isFinite(count) ? count : 0;
    }
  ...
  async function reconcileFloatWindow(revision) {
    const result = await storageGet([
      ...
      GW_UNREAD_COUNTS_KEY, GW_MAILBOX_CACHE_KEY
    ]);
    ...
    moeEmailCacheSnapshot = Array.isArray(result[MOE_EMAIL_CACHE_KEY])
      ...
      : [];
    gwUnreadSnapshot = result[GW_UNREAD_COUNTS_KEY] || {};
    gwMailboxCacheSnapshot = Array.isArray(result[GW_MAILBOX_CACHE_KEY])
      ? result[GW_MAILBOX_CACHE_KEY]
      : [];
  ...
  function handleStorageChanged(changes, area) {
    ...
    if (changes[GW_UNREAD_COUNTS_KEY]) {
      gwUnreadSnapshot = changes[GW_UNREAD_COUNTS_KEY].newValue || {};
      ballDataChanged = true;
    }
    if (changes[GW_MAILBOX_CACHE_KEY]) {
      gwMailboxCacheSnapshot = Array.isArray(changes[GW_MAILBOX_CACHE_KEY].newValue)
        ? changes[GW_MAILBOX_CACHE_KEY].newValue
        : [];
      ballDataChanged = true;
    }
